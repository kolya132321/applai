...
from api_objects_custom import (
    URN,
    Conversation,
...
from linkedin_messaging.exceptions import TooManyRequestsError
...
PROXY=f"http://13.51.109.253:8888"
...
class LinkedInMessaging:
    ...
    async def _get(self, relative_url: str, **kwargs: Any) -> aiohttp.ClientResponse:
        return await self.session.get(API_BASE_URL + relative_url,proxy=PROXY, **kwargs)
    ...
    async def login(self, email: str, password: str, new_session: bool = True):
        ...
        # Get the CSRF token.
        async with self.session.get(SEED_URL,proxy=PROXY) as seed_response:
    ...
    async def download_linkedin_media(self, url: str) -> bytes:
        async with self.session.get(url,proxy=PROXY) as media_resp:
    ...
    async def download_profile_picture(self, picture: Picture) -> bytes:
        ...
        async with await self.session.get(url,proxy=PROXY) as profile_resp:
